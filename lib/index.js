...
function initCanvas() {
  ...
  gameloop()
  let key = spaceship1.time;
  let score = spaceship1.score;
  let level = spaceship1.level;
  levelValue.innerText = spaceship1.level;
  currentScoreValue.innerText = spaceship1.currentScore;
  setGameId(key, spaceship1)
}
...
function setGameId(key, spaceship) {
  var stringifiedNewIdeaObject = JSON.stringify(spaceship);
  localStorage.setItem(key, stringifiedNewIdeaObject);
}

function getGameId(key) {
  var storedGame = localStorage.getItem(key);
  var gameObject = JSON.parse(storedGame);
  return gameObject;
}
